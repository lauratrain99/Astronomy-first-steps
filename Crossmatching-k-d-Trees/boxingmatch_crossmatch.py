...
It only loops through objects in the second catalogue 
with declinations between dec1 - max_radius and dec1 + max_radius using binary search.
It returns:
...
def crossmatch(cat1, cat2, max_radius):
  ...
  for point in range(cat1.shape[0]):
      ...
      start_point = np.searchsorted(cat2[:,1], dec1-max_radius, side ='left')
      end_point = np.searchsorted(cat2[:,1], dec1+max_radius, side ='left')
      distances = []
